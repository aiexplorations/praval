...
class MemoryStorageAdapter:
    ...
    async def store_memory_as_data(
        self,
        ...
        try:
            ...
            if success:
                ...
                return StorageResult(success=False, error="Failed to store memory")

        except Exception as e:
            logger.error(f"Memory storage failed: {e}")
            return StorageResult(
                success=False, error=f"Memory storage failed: {str(e)}"
            )

    async def retrieve_memory_as_data(
        ...
        try:
            ...
            if not memory_entry and self.memory_manager.long_term_memory:
                ...
                memory_entry = await self.memory_manager.long_term_memory.get_memory(
                    memory_id
                )
            ...
            return StorageResult(
                success=False, error=f"Memory retrieval failed: {str(e)}"
            )

    async def search_memory_as_data(
        self,
        query: str,
        ...
        try:
            ...
            return StorageResult(success=False, error=f"Memory search failed: {str(e)}")
...
class UnifiedDataInterface:
    ...
    async def store(self, location: str, data: Any, **kwargs) -> StorageResult:
        """
        ...
        if location.startswith("memory:"):
            ...
            if not self.memory_adapter:
                return StorageResult(success=False, error="Memory system not available")
    ...
    async def get(self, location: str, **kwargs) -> StorageResult:
        """
    ...
    async def resolve_reference(
        self, reference: Union[str, DataReference], **kwargs
    ) -> StorageResult:
        """
        ...
        if isinstance(reference, str):
            if reference.startswith("memory:"):
                ...
                if self.data_manager:
                    return await self.data_manager.resolve_data_reference(
                        reference, **kwargs
                    )
                else:
                    return StorageResult(
            ...
            return StorageResult(
                success=False, error=f"Invalid reference type: {type(reference)}"
            )
