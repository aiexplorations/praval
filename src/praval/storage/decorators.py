...
from .exceptions import StorageConfigurationError, StorageNotFoundError
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    loop.create_task(
                        _ensure_storage_providers(func._storage_config, func.__name__)
                    )
                else:
            ...
            for provider_name in provider_names:
                try:
                    ...
                    raise StorageConfigurationError(
                        provider_name,
                        (
                            f"Required provider '{provider_name}' not available "
...
async def _ensure_storage_providers(config: Dict[str, Any], agent_name: str):
    ...
    if isinstance(providers, str):
        ...
        for provider_name, provider_config in providers.items():
            ...
            await _ensure_single_provider(
                registry, provider_name, {"default_configs": merged_config}, agent_name
            )


async def _ensure_single_provider(
    registry, provider_name: str, config: Dict[str, Any], agent_name: str
):
    ...
    try:
        ...
        if config.get("auto_register", True):
            ...
            raise StorageConfigurationError(
                provider_name,
                (
                    f"Storage provider '{provider_name}' not found and "
                    f"auto_register is disabled"
                ),
            )


async def _auto_register_provider(
    registry, provider_name: str, config: Dict[str, Any], agent_name: str
):
    ...
    if provider_name.lower() not in provider_classes:
        raise StorageConfigurationError(
            provider_name, f"Unknown storage provider type: {provider_name}"
        )
    ...
    env_mappings = {
        ...
        "filesystem": {"base_path": "FILESYSTEM_BASE_PATH"},
        "qdrant": {
            "url": "QDRANT_URL",
    ...
    try:
        ...
        permissions = (
            config.get("permissions", [agent_name]) if agent_name != "unknown" else None
        )
        success = await registry.register_provider(
            provider_instance, permissions=permissions, auto_connect=True
        )
        ...
        if success:
            ...
            raise StorageConfigurationError(
                provider_name, f"Failed to register storage provider: {provider_name}"
            )

    except Exception as e:
        raise StorageConfigurationError(
            provider_name, f"Failed to create storage provider '{provider_name}': {e}"
        )
