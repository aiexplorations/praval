...
class PostgreSQLProvider(BaseStorageProvider):
    ...
    def _initialize(self):
        ...
        if not ASYNCPG_AVAILABLE:
            raise ImportError(
                (
                    "asyncpg is required for PostgreSQL provider. Install with:"
                    " pip install asyncpg"
                )
            )
        ...
        try:
            async with self.connection_pool.acquire() as conn:
                if isinstance(data, dict):
                    ...
                    query = (
                        f"INSERT INTO {resource} ({', '.join(columns)}) VALUES "
                        f"({', '.join(placeholders)})"
                    )
    ...
    async def _execute_structured_query(
        self, conn, resource: str, query_dict: Dict, **kwargs
    ) -> StorageResult:
