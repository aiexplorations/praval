...
class S3Provider(BaseStorageProvider):
                ...
                if error_code == 404:
                    ...
                    if self.config.get("create_bucket", False):
                        ...
                        raise StorageConnectionError(
                            self.name,
                            (
                                f"Bucket '{self.bucket_name}' does not exist and "
                                f"create_bucket is disabled"
                            ),
                        )
                else:
                    raise
    ...
    async def store(self, resource: str, data: Any, **kwargs) -> StorageResult:
        ...
        try:
            ...
            head_response = self.s3_client.head_object(
                Bucket=self.bucket_name, Key=resource
            )
    ...
    async def retrieve(self, resource: str, **kwargs) -> StorageResult:
        ...
        try:
            get_kwargs = {"Bucket": self.bucket_name, "Key": resource}
            ...
            if error_code == "NoSuchKey":
                return StorageResult(
                    success=False, error=f"Object '{resource}' not found"
                )
            else:
        ...
        try:
            if isinstance(query, str):
                if query == "list":
                    ...
                    list_kwargs = {"Bucket": self.bucket_name, "Prefix": resource}
        ...
        try:
            if kwargs.get("recursive", False):
                ...
                list_response = self.s3_client.list_objects_v2(
                    Bucket=self.bucket_name, Prefix=resource
                )
                ...
                objects_to_delete = [
                    {"Key": obj["Key"]} for obj in list_response["Contents"]
                ]
