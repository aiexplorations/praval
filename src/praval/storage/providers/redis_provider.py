...
class RedisProvider(BaseStorageProvider):
    ...
    def _initialize(self):
        ...
        if not REDIS_AVAILABLE:
            raise ImportError(
                "redis is required for Redis provider. Install with: pip install redis"
            )
    ...
    async def store(self, resource: str, data: Any, **kwargs) -> StorageResult:
        ...
        try:
            ...
            result = await self.redis_client.set(
                resource, serialized_data, ex=ex, px=px, nx=nx, xx=xx
            )
    ...
    async def _execute_hash_operation(
        self, operation: str, key: str, **kwargs
    ) -> StorageResult:
    ...
    async def _execute_list_operation(
        ...
        if operation == "lrange":
            ...
            return StorageResult(
                success=True, data={"length": length}, metadata={"operation": "llen"}
            )
    ...
    async def _execute_set_operation(
        ...
        if operation == "smembers":
            ...
            return StorageResult(
                success=True, data={"count": count}, metadata={"operation": "scard"}
            )
