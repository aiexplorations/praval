...
from ..exceptions import StorageConfigurationError, StorageConnectionError
...
class FileSystemProvider(BaseStorageProvider):
        ...
        if not self.base_path.exists():
            if self.config["create_directories"]:
                ...
                raise StorageConfigurationError(
                    self.name, f"Base path does not exist: {self.base_path}"
                )

        if not self.base_path.is_dir():
            raise StorageConfigurationError(
                self.name, f"Base path is not a directory: {self.base_path}"
            )
    ...
    async def retrieve(self, resource: str, **kwargs) -> StorageResult:
        ...
        try:
            ...
            if not file_path.exists():
                return StorageResult(success=False, error=f"File not found: {resource}")

            if not file_path.is_file():
                return StorageResult(
                    success=False, error=f"Path is not a file: {resource}"
                )
            ...
            if binary_mode:
                ...
                if decode_json and (
                    suffix == ".json" or content_type == "application/json"
                ):
                    try:
            ...
            if isinstance(query, str):
                if query == "list":
                    ...
                    if not dir_path.exists():
                        return StorageResult(
                            success=False, error=f"Directory not found: {resource}"
                        )

                    if not dir_path.is_dir():
                        return StorageResult(
                            success=False, error=f"Path is not a directory: {resource}"
                        )
                    ...
                    base_dir = (
                        self._resolve_path(resource) if resource else self.base_path
                    )
                    ...
                    if not path.exists():
                        return StorageResult(
                            success=False, error=f"Path not found: {resource}"
                        )
    ...
    async def delete(self, resource: str, **kwargs) -> StorageResult:
        ...
        try:
            ...
            if not path.exists():
                return StorageResult(success=False, error=f"Path not found: {resource}")
            ...
            if path.is_file():
                ...
                return StorageResult(
                    success=False, error=f"Unknown path type: {resource}"
                )
