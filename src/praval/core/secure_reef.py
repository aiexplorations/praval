...
class SecureReef:
    ...
    async def initialize(self, agent_name: str):
        ...
        await self.key_registry.register_agent(
            agent_name, self.key_manager.get_public_keys()
        )
        ...
        try:
            ...
            logger.info(
                (
                    f"Secure reef initialized for agent '{agent_name}' using "
    ...
    async def _setup_subscriptions(self):
        ...
        if self.protocol == TransportProtocol.AMQP:
            ...
            await self.transport.subscribe(
                f"agent.{self.agent_name}.*", self._handle_incoming_message
            )
            # Subscribe to broadcasts
            await self.transport.subscribe("broadcast.*", self._handle_incoming_message)
    ...
    async def send_secure_spore(
        ...
        try:
            ...
            logger.debug(
                f"Sent secure spore {secure_spore.id} to {to_agent or 'broadcast'}"
            )
    ...
    def _generate_topic(self, recipient: Optional[str], message_type: str) -> str:
        ...
        if self.protocol == TransportProtocol.AMQP:
            return (
                f"agent.{recipient}.{message_type}"
                if recipient
                else f"broadcast.{message_type}"
            )
        elif self.protocol == TransportProtocol.MQTT:
            return (
                f"agent/{recipient}/{message_type}"
    ...
    async def _decrypt_spore(self, secure_spore: SecureSpore) -> Dict[str, Any]:
        ...
        if not sender_keys:
            raise ValueError(
                f"No public keys found for sender: {secure_spore.from_agent}"
            )
    ...
    async def broadcast(
        self, knowledge: Dict[str, Any], spore_type: SporeType = SporeType.BROADCAST
    ) -> str:
        """Broadcast knowledge to all agents."""
        return await self.send_secure_spore(
            to_agent=None, knowledge=knowledge, spore_type=spore_type
        )

    async def request(
        self, to_agent: str, request: Dict[str, Any], expires_in_seconds: int = 300
    ) -> str:
    ...
    async def reply(
        self, to_agent: str, response: Dict[str, Any], reply_to_spore_id: str
    ) -> str:
        """Reply to a request."""
        response_data = {**response, "reply_to": reply_to_spore_id}

        return await self.send_secure_spore(
            to_agent=to_agent, knowledge=response_data, spore_type=SporeType.RESPONSE
        )
    ...
    async def rotate_keys(self):
        ...
        _ = self.key_manager.get_public_keys()
        rotation_result = self.key_manager.rotate_keys()

        # Update key registry
        await self.key_registry.register_agent(
            self.agent_name, self.key_manager.get_public_keys()
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "registered_agents": (
                len(self.key_registry._keys) if self.key_registry else 0
            ),
        }
...
async def initialize_secure_reef(
    agent_name: str,
    protocol: TransportProtocol = TransportProtocol.AMQP,
    transport_config: Optional[Dict[str, Any]] = None,
) -> SecureReef:
...
class SecureReefAdapter:
    ...
    def send(self, from_agent: str, to_agent: str, knowledge: Dict, **kwargs) -> str:
        ...
        return asyncio.run(
            self.secure_reef.send(from_agent, to_agent, knowledge, **kwargs)
        )
