...
class MessageTransport(ABC):
    ...
    @abstractmethod
    async def publish(
        self, topic: str, message: bytes, priority: int = 5, ttl: Optional[int] = None
    ) -> None:
    ...
    def _create_tls_context(self, config: Dict[str, Any]) -> ssl.SSLContext:
        ...
        if client_cert := config.get("client_cert"):
            ssl_context.load_cert_chain(client_cert, keyfile=config.get("client_key"))
...
class AMQPTransport(MessageTransport):
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        try:
            self.connection = await self._aio_pika.connect_robust(
                **connection_params,
    ...
    async def publish(
        ...
        When a Spore is provided, it uses the native AMQP format (Spore as wire
        protocol).
        ...
        if self._aio_pika is None:
            raise PublishError("AMQP transport dependency not initialized")
        ...
        try:
            ...
            if hasattr(message, "to_amqp_message"):
                ...
                amqp_message = self._aio_pika.Message(
                    message,
            ...
            if not hasattr(self.exchange, "publish"):
                logger.debug(
                    "AMQP exchange mock without publish(); skipping publish call"
                )
                return
    ...
    async def subscribe(self, topic: str, callback: Callable) -> None:
        ...
        The handler is called with either a Spore object or bytes, depending on the
        callback signature.
        """
...
class MQTTTransport(MessageTransport):
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(
                (
                    f"MQTT transport initialized: "
                    f"{client_params['hostname']}:{client_params['port']}"
                )
            )
    ...
    async def publish(
        ...
        qos_map = {
            ...
            2: 0,
            3: 0,
            4: 0,  # Low priority -> QoS 0
            5: 1,
            6: 1,
            7: 1,  # Medium priority -> QoS 1
            8: 2,
            9: 2,
            10: 2,  # High priority -> QoS 2
        }
        ...
        try:
            ...
            await self.client.publish(
                topic=full_topic, payload=message, qos=qos, retain=False
            )
...
class STOMPTransport(MessageTransport):
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(
                (
                    f"STOMP transport initialized: "
                    f"{config.get('host', 'localhost')}:{config.get('port', 61614)}"
                )
            )
    ...
    async def publish(
        ...
        try:
            ...
            headers = {"priority": str(priority), "persistent": "true"}
