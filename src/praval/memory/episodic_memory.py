...
class EpisodicMemory:
    ...
    def store_experience(
        ...
        Args:
            ...
            experience_type: Type of experience (e.g., "task_completion",
            "problem_solving")
    ...
    def get_conversation_context(
        self, agent_id: str, turns: Optional[int] = None
    ) -> List[MemoryEntry]:
        """
        ...
        recent_memories = self.short_term_memory.get_recent(
            agent_id=agent_id, limit=turns * 2
        )
        conversation_memories = [
            m
            for m in recent_memories
            if m.memory_type == MemoryType.EPISODIC
            and m.metadata.get("type") == "conversation_turn"
        ]
        ...
        lt_conversations = [
            m
            for m in search_result.entries
    ...
    def get_similar_experiences(
        self, agent_id: str, experience_description: str, limit: int = 5
    ) -> MemorySearchResult:
        """
    ...
    def archive_old_episodes(self, cutoff_days: Optional[int] = None):
        ...
        _ = datetime.now() - timedelta(days=cutoff_days)
