...
class MemoryManager:
    ...
    def __init__(
        ...
        if backend in ["auto", "chromadb", "embedded"]:
            try:
                # Enable separated knowledge vs conversational memory collections.
        ...
        if vector_store:
            self.episodic_memory = EpisodicMemory(
                long_term_memory=vector_store, short_term_memory=self.short_term_memory
            )
            self.semantic_memory = SemanticMemory(long_term_memory=vector_store)
        else:
    ...
    def store_memory(
        ...
        if store_long_term and vector_store:
            try:
                ...
                logger.debug(
                    (
                        f"Memory {memory_id} stored in both short-term and "
                        f"persistent memory"
                    )
                )
            except Exception as e:
                logger.error(
                    f"Failed to store memory {memory_id} in persistent storage: {e}"
                )
    ...
    def search_memories(self, query: MemoryQuery) -> MemorySearchResult:
        ...
        if vector_store:
            try:
                ...
                persistent_results = MemorySearchResult(
    ...
    def get_domain_knowledge(
        self, agent_id: str, domain: str, limit: int = 20
    ) -> List[MemoryEntry]:
        """
    ...
    def clear_agent_memories(
        self, agent_id: str, memory_types: Optional[List[MemoryType]] = None
    ):
        """
    ...
    def health_check(self) -> Dict[str, bool]:
        ...
        if vector_store:
            ...
            health["episodic_memory"] = health[
                "persistent_memory"
            ]  # Depends on persistent
            health["semantic_memory"] = health[
                "persistent_memory"
            ]  # Depends on persistent
    ...
    def get_active_backend(self) -> Dict[str, Any]:
        ...
        result = {
            "name": self.backend,
            "type": (
                "persistent" if self.backend in ["chromadb", "qdrant"] else "in_memory"
            ),
            "available": True,
            "details": {},
        }
    ...
    def _combine_search_results(
        self, results: List[tuple], query: MemoryQuery
    ) -> MemorySearchResult:
    ...
    def _index_knowledge_base(self):
        ...
        try:
            ...
            if hasattr(vector_store, "index_knowledge_files"):
                indexed_count = vector_store.index_knowledge_files(
                    kb_path, self.agent_id
                )
                logger.info(
                    (
                        f"Indexed {indexed_count} knowledge base files for agent "
                        f"{self.agent_id}"
                    )
                )
            else:
    ...
    def get_knowledge_references(
        self, content: str, importance_threshold: float = 0.7
    ) -> List[str]:
