...
class ShortTermMemory:
    ...
    def __init__(
        ...
        self._agent_memories: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=max_entries // 10)
        )
    ...
    def search(self, query: MemoryQuery) -> MemorySearchResult:
        ...
        with self._lock:
            ...
            if query.memory_types:
                candidates = [
                    m for m in candidates if m.memory_type in query.memory_types
                ]

            # Filter by temporal constraints
            if query.temporal_filter:
                candidates = self._apply_temporal_filter(
                    candidates, query.temporal_filter
                )
            ...
            sorted_results = sorted(
                zip(matching_entries, scores), key=lambda x: x[1], reverse=True
            )
            matching_entries, scores = (
    ...
    def get_recent(
        ...
        with self._lock:
            if agent_id:
                ...
                memories = [
                    self._memories[mid]
                    for mid in reversed(agent_memory_ids)
                    if mid in self._memories
                ]
            else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "memory_types": {
                    mtype.value: sum(
                        1 for m in self._memories.values() if m.memory_type == mtype
                    )
    ...
    def _apply_temporal_filter(
        self, memories: List[MemoryEntry], temporal_filter: Dict[str, datetime]
    ) -> List[MemoryEntry]:
        ...
        if "before" in temporal_filter:
            filtered = [
                m for m in filtered if m.created_at <= temporal_filter["before"]
            ]
    ...
    def _cleanup_old_memories(self):
        ...
        old_memory_ids = [
            ...
            and memory.importance < 0.8  # Keep important memories longer
        ]
