...
def agent_pipeline(*agents: Callable, channel: str = "pipeline") -> Callable:
    ...
    for agent_func in agents:
        if not hasattr(agent_func, "_praval_agent"):
            raise ValueError(
                f"Function {agent_func.__name__} is not decorated with @agent"
            )
...
def conditional_agent(condition_func: Callable[[Any], bool]):
    ...
    def decorator(agent_func: Callable) -> Callable:
        if not hasattr(agent_func, "_praval_agent"):
            raise ValueError(
                "conditional_agent must be applied to @agent decorated functions"
            )
...
def throttled_agent(delay_seconds: float):
    ...
    def decorator(agent_func: Callable) -> Callable:
        if not hasattr(agent_func, "_praval_agent"):
            raise ValueError(
                "throttled_agent must be applied to @agent decorated functions"
            )
...
class AgentSession:
    ...
    def broadcast(self, data: Dict[str, Any]) -> str:
        ...
        return reef.system_broadcast(
            {**data, "_session": self.session_name}, self.channel_name
        )
...
def run_agents(
    *agent_funcs: Callable,
    ...
    return _run_agents_impl(
        *agent_funcs, backend_config=backend_config, channel_queue_map=channel_queue_map
    )
