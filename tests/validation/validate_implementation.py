    ...
    try:
        ...
        with patch("praval.core.agent.ProviderFactory") as mock_factory:
            ...
            with patch.dict("os.environ", {"OPENAI_API_KEY": "test_key"}):
                ...
                assert (
                    len(agent.conversation_history) == 2
                ), "Conversation history length incorrect"
    ...
    try:
        ...
        with patch("praval.core.agent.ProviderFactory") as mock_factory:
            ...
            with patch.dict("os.environ", {"OPENAI_API_KEY": "test_key"}):
                ...
                assert (
                    tool_info["function"] == add_numbers
                ), "Tool function not stored correctly"
                assert (
                    "Add two numbers together" in tool_info["description"]
                ), "Tool description not stored"
            ...
            with patch("praval.core.agent.ProviderFactory") as mock_factory:
                ...
                with patch("praval.core.storage.StateStorage") as mock_storage_class:
                    ...
                    with patch.dict("os.environ", {"OPENAI_API_KEY": "test_key"}):
                        ...
                        mock_storage.save.assert_called_with(
                            "persistent_agent", agent.conversation_history
                        )
...
def main():
    ...
    if passed == total:
        print(
            (
                "🎉 All validation tests passed! Praval Phase 1 implementati"
