...
Tests real functionality without mocks, following the "Real Implementations Only"
principle.
"""
        ...
        for invalid_config, expected_error in validation_tests:
            try:
                ...
                assert expected_error in str(
                    e
                ), f"Wrong error message for {invalid_config}"
...
def test_tool_registration_real():
    ...
    try:
        ...
        assert (
            "sum of two numbers" in sum_tool["description"]
        ...
        assert (
            sum_tool["parameters"]["x"]["type"] == "int"
        ), "Parameter type not captured"
        assert (
            sum_tool["parameters"]["x"]["required"] is True
        ), "Required parameter not marked"

        format_tool = agent.tools["format_text"]
        assert (
            format_tool["parameters"]["uppercase"]["required"] is False
        ), "Optional parameter marked as required"
        ...
        try:
            ...
            assert "return type hint" in str(
                e
            ), "Wrong error message for missing return type hint"
...
def test_exception_hierarchy():
    ...
    try:
        ...
        # Test inheritance
        assert issubclass(
            ProviderError, PravalError
        ), "ProviderError should inherit from PravalError"
        assert issubclass(
            ConfigurationError, PravalError
        ), "ConfigurationError should inherit from PravalError"
        assert issubclass(
            ToolError, PravalError
        ), "ToolError should inherit from PravalError"
        assert issubclass(
            StateError, PravalError
        ), "StateError should inherit from PravalError"
