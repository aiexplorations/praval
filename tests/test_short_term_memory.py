...
class TestShortTermMemoryInitialization:
    ...
    def test_short_term_memory_custom_initialization(self):
        ...
        memory = ShortTermMemory(
            max_entries=500, retention_hours=12, cleanup_interval=1800
        )
...
class TestShortTermMemoryStorage:
    ...
    def test_store_memory_thread_safety(self):
        ...
        # All entries should be stored (cleanup only removes OLD memories, not fresh
        # ones)
...
class TestShortTermMemorySearch:
    ...
    def test_search_agent_specific(self):
        ...
        query = MemoryQuery(
            query_text="fox", agent_id="agent_2", similarity_threshold=0.2
        )
        result = self.memory.search(query)
    ...
    def test_search_similarity_threshold(self):
        ...
        high_threshold_query = MemoryQuery(
            query_text="fox", similarity_threshold=0.9  # Very high threshold
        )
        low_threshold_query = MemoryQuery(
            query_text="fox", similarity_threshold=0.1  # Very low threshold
        )
    ...
    def test_search_limit(self):
        ...
        query = MemoryQuery(
            query_text="the", limit=2  # Common word likely to match multiple entries
        )
        result = self.memory.search(query)
    ...
    def test_search_no_matches(self):
        ...
        query = MemoryQuery(
            query_text="nonexistent_unique_term_xyz123", similarity_threshold=0.8
        )
        result = self.memory.search(query)
...
class TestShortTermMemoryIntegration:
    ...
    def test_full_memory_lifecycle(self):
        ...
        # Access some memories (access_count is 2 because search already marked them
        # accessed)
        for i in range(3):
            ...
            )  # Already accessed by search, now accessed again
