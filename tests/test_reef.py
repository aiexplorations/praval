...
from praval.core.reef import Reef, ReefChannel, Spore, SporeType, get_reef
...
class TestReef:
    ...
    def test_request_and_reply(self):
        ...
        request_id = reef.request(
            from_agent="client",
            to_agent="service",
            request={"query": "calculate_fibonacci", "params": {"n": 10}},
        ...
        _ = reef.reply(
            from_agent="service",
            to_agent="client",
            response={"result": 55, "computation_time": 0.001},
...
class TestReefIntegration:
    ...
    def test_concurrent_access(self):
        ...
        def sender_thread():
            for i in range(10):
                reef.send(
                    from_agent="sender", to_agent="receiver", knowledge={"index": i}
                )
                results["sent"] += 1
    ...
    def test_error_handling_in_handlers(self):
        ...
        reef.broadcast(from_agent="sender", knowledge={"test": "error handling"})
