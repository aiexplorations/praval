...
class TestKnowledgeBaseBenchmark:
    ...
    def create_test_knowledge_base(self, num_docs: int = 10) -> List[Path]:
        ...
        for i in range(num_docs):
            ...
            ## Abstract
            This document explores {topic} and their applications in modern AI systems.
            The research covers theory, implementations, and real-world applications.

            ## Introduction
            {topic.title()} represent a major advancement in machine learning and AI.
            This analysis examines key principles, methods, and results.
            ...
            ## Conclusion
            The research demonstrates that {topic} provide valuable AI capabilities.
            Future work should focus on optimization and practical deployment.
    ...
    def create_pdf_files(self, num_pdfs: int = 3) -> List[Path]:
        ...
        pdf_contents = [
            [
                ...
                "",
                (
                    "This comprehensive guide covers the fundamentals of deep r"
                ...
                (
                    "Reinforcement learning is a machine learning paradigm wher"
                    "e agents learn"
                ),
                (
                    "to make decisions by interacting with an environment and r"
                    "eceiving rewards."
                ),
                "",
            ...
            [
                ...
                "",
                (
                    "The transformer architecture has revolutionized natural la"
                    "nguage processing,"
                ),
            ...
            [
                ...
                "",
                (
                    "This research paper examines the evolution of computer vis"
                    "ion techniques,"
                ),
    ...
    def test_large_knowledge_base_performance(self):
        ...
        # create_test_knowledge_base(50) creates 50 .md files + min(3, 50//3) = 3 .py
        # files = 53 total
        ...
        assert (
            sum(indexing_times) / len(indexing_times) < 0.1
    ...
    def test_memory_manager_knowledge_base_integration(self):
        ...
        _ = self.create_test_knowledge_base(5)

        # Mock MemoryManager to avoid external dependencies
        with patch(
            "praval.memory.memory_manager.EmbeddedVectorStore"
        ) as mock_store_class:
            mock_store = Mock()
            ...
            indexed_count = mock_store.index_knowledge_files(
                self.temp_dir, self.agent_id
            )
    ...
    def test_memory_search_performance(self):
        ...
        def mock_search(query: MemoryQuery) -> MemorySearchResult:
            ...
            for entry in indexed_entries:
                content_lower = entry.content.lower()
                score = sum(1 for term in query_terms if term in content_lower) / len(
                    query_terms
                )
        ...
        for query_text in test_queries:
            query = MemoryQuery(
                query_text=query_text, limit=5, similarity_threshold=0.3
            )
    ...
    def test_knowledge_base_metadata_tracking(self):
        ...
        # create_test_knowledge_base(3) creates 3 .md files + min(3, 3//3) = 1 .py file
        # = 4 total
        expected_files = len(files)
        assert count == expected_files
        assert len(indexed_entries) == expected_files
...
if __name__ == "__main__":
    ...
    pytest.main([__file__, "-v", "--tb=short", "-m", "knowledge_base"])
