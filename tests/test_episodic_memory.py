...
class TestEpisodicMemoryConversationStorage:
    ...
    def test_conversation_importance_calculation_basic(self):
        ...
        self.memory.store_conversation_turn(
            agent_id="importance_agent", user_message="Hi", agent_response="Hello"
        )
    ...
    def test_conversation_importance_calculation_length_bonus(self):
        ...
        # Long conversation should get importance bonus (total > 500 chars for +0.2
        # bonus)
    ...
    def test_conversation_importance_calculation_max_capped(self):
        ...
        very_long_message = ("This is an extremely long message " * 50) + (
            " with critical urgent important problem error help learn r"
            "emember goal plan decision"
        )
        very_long_response = ("This is an extremely long response " * 50) + (
            " addressing critical urgent important problem error help l"
            "earn remember goal plan decision"
        )
...
class TestEpisodicMemoryExperienceStorage:
    ...
    def test_store_experience_successful(self):
        ...
        lt_call_args = self.mock_long_term.store.call_args[0][0]
        assert (
            lt_call_args.content
            == "task_completion: Successfully generated and tested sorting function"
        )
        assert lt_call_args.metadata["experience_type"] == "task_completion"
        assert lt_call_args.metadata["experience_data"] == experience_data
        assert (
            lt_call_args.metadata["outcome"]
            == "Successfully generated and tested sorting function"
        )
...
class TestEpisodicMemoryConversationContext:
    ...
    def create_conversation_memory(
        self, memory_id: str, agent_id: str, user_msg: str, agent_msg: str
    ):
    ...
    def test_get_conversation_context_from_short_term(self):
        ...
        # Mock short-term memory to return conversation memories with explicit
        # timestamps
        # so sorting by created_at (most recent first) produces predictable results
        conv_3 = self.create_conversation_memory(
            "conv_3", "context_agent", "How are you?", "I'm fine"
        )
        conv_3.created_at = datetime.now()  # Most recent

        conv_2 = self.create_conversation_memory(
            "conv_2", "context_agent", "What's up?", "Not much"
        )
        conv_2.created_at = datetime.now() - timedelta(minutes=5)

        conv_1 = self.create_conversation_memory(
            "conv_1", "context_agent", "Hello", "Hi there"
        )
        ...
        # Should return only conversation memories, sorted by created_at (most recent
        # first)
        assert len(result) == 2
        ...
        self.mock_short_term.get_recent.assert_called_once_with(
            agent_id="context_agent", limit=4
        )  # turns * 2
    ...
    def test_get_conversation_context_fallback_to_long_term(self):
        ...
        short_term_conv = [
            self.create_conversation_memory(
                "st_conv_1", "fallback_agent", "Recent msg", "Recent response"
            )
        ]
        ...
        long_term_conversations = [
            self.create_conversation_memory(
                "lt_conv_2", "fallback_agent", "Old msg 2", "Old response 2"
            ),
            self.create_conversation_memory(
                "lt_conv_1", "fallback_agent", "Old msg 1", "Old response 1"
            ),
        ]
    ...
    def test_get_conversation_context_deduplication(self):
        ...
        duplicate_conv = self.create_conversation_memory(
            "dup_conv", "dup_agent", "Duplicate", "Response"
        )
    ...
    def test_get_conversation_context_chronological_ordering(self):
        ...
        old_conv = self.create_conversation_memory(
            "old_conv", "chrono_agent", "Old", "Old response"
        )
        old_conv.created_at = old_time

        recent_conv = self.create_conversation_memory(
            "recent_conv", "chrono_agent", "Recent", "Recent response"
        )
    ...
    def test_get_conversation_context_default_window(self):
        ...
        self.mock_long_term.search.return_value = MemorySearchResult(
        ...
        self.mock_short_term.get_recent.assert_called_once_with(
            agent_id="default_agent", limit=100
        )  # 50 * 2
...
class TestEpisodicMemoryExperienceRetrieval:
    ...
    def create_experience_memory(
        self, memory_id: str, agent_id: str, exp_type: str, outcome: str
    ):
    ...
    def test_get_similar_experiences_basic(self):
        ...
        experience_memories = [
            self.create_experience_memory(
                "exp_1", "exp_agent", "problem_solving", "Solved algorithm issue"
            ),
            self.create_experience_memory(
                "exp_2", "exp_agent", "code_review", "Found performance bug"
            ),
        ]
        ...
        mock_search_result = MemorySearchResult(
            entries=all_memories, scores=[0.9, 0.8, 0.7], query=Mock(), total_found=3
        )
        self.mock_long_term.search.return_value = mock_search_result
        ...
        assert all(
            entry.metadata.get("type") == "experience" for entry in result.entries
        )
    ...
    def test_get_similar_experiences_no_experiences_found(self):
        ...
        mock_search_result = MemorySearchResult(
            entries=[conversation_memory], scores=[0.8], query=Mock(), total_found=1
        )
        self.mock_long_term.search.return_value = mock_search_result
...
class TestEpisodicMemoryTimelineRetrieval:
    ...
    def test_get_episode_timeline_basic(self):
        ...
        result = self.memory.get_episode_timeline(
            agent_id="timeline_agent", start_time=start_time, end_time=end_time
        )
        ...
        assert search_call_args.temporal_filter == {
            "after": start_time,
            "before": end_time,
        }
    ...
    def test_get_episode_timeline_empty_results(self):
        ...
        result = self.memory.get_episode_timeline(
            agent_id="empty_timeline_agent", start_time=start_time, end_time=end_time
        )
...
class TestEpisodicMemoryIntegration:
    ...
    def test_importance_calculation_edge_cases(self):
        ...
        self.memory.store_conversation_turn(
            agent_id="empty_agent", user_message="", agent_response=""
        )
