...
class TestAgentDecorator:
    ...
    def test_agent_decorator_hitl_disabled_by_default(self):
        """Test that HITL is disabled by default on @agent."""

        @agent("no_hitl_agent")
        def test_func(spore):
            return {"result": "test"}

        assert test_func._praval_hitl_enabled is False
        assert get_agent_info(test_func)["hitl"] is False

    def test_agent_decorator_hitl_enabled_flag(self):
        """Test that HITL can be enabled with hitl=True."""

        @agent("hitl_agent", hitl=True)
        def test_func(spore):
            return {"result": "test"}

        assert test_func._praval_hitl_enabled is True
        assert get_agent_info(test_func)["hitl"] is True
...
class TestAgentHandler:
    ...
    @patch("praval.decorators.Agent")
    def test_handler_responds_to_filtering_allows_matching_types(
        self, mock_agent_class
    ):
    ...
    @patch("praval.decorators.Agent")
    def test_handler_responds_to_filtering_blocks_non_matching_types(
        self, mock_agent_class
    ):
    ...
    @patch("praval.decorators.Agent")
    def test_handler_resolves_knowledge_references_when_memory_enabled(
        self, mock_agent_class
    ):
    ...
    def test_handler_auto_broadcast_when_enabled(self, mock_agent_class):
        ...
        mock_agent.broadcast_knowledge.assert_called_once_with(
            expected_data, channel="test_channel"
        )
...
class TestChatFunction:
    ...
    def test_chat_requires_agent_context(self):
        ...
        with pytest.raises(
            RuntimeError,
            match="chat\\(\\) can only be used within @agent decorated functions",
        ):
            chat("test message")
...
class TestAChatFunction:
    ...
    async def test_achat_requires_agent_context(self):
        ...
        with pytest.raises(
            RuntimeError,
            match="achat\\(\\) can only be used within @agent decorated functions",
        ):
...
class TestBroadcastFunction:
    ...
    def test_broadcast_requires_agent_context(self):
        ...
        with pytest.raises(
            RuntimeError,
            match="broadcast\\(\\) can only be used within @agent decorated functions",
        ):
            broadcast({"test": "data"})
    ...
    def test_broadcast_with_custom_channel(self):
        ...
        mock_agent.broadcast_knowledge.assert_called_once_with(
            {"test": "data"}, channel="custom_channel"
        )
    ...
    def test_broadcast_adds_message_type(self):
        ...
        # broadcast() defaults to reef's default channel ('main') for agent chaining
        mock_agent.broadcast_knowledge.assert_called_once_with(
            {"content": "hello", "type": "greeting"}, channel="main"
        )
    ...
    def test_broadcast_overwrites_existing_type(self):
        ...
        # broadcast() defaults to reef's default channel ('main') for agent chaining
        mock_agent.broadcast_knowledge.assert_called_once_with(
            {"type": "new_type", "data": "test"}, channel="main"
        )
