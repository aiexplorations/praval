...
class TestAgentPipeline:
    ...
    def test_agent_pipeline_validates_agents(self):
        ...
        with pytest.raises(
            ValueError, match="Function not_an_agent is not decorated with @agent"
        ):
    ...
    def test_pipeline_trigger_subscribes_agents(
        self, mock_get_agent_info, mock_get_reef
    ):
        ...
        _ = pipeline({"initial": "data"})

        # Verify all agents were subscribed to pipeline channel
        for mock_info in mock_agent_infos:
            mock_info["underlying_agent"].subscribe_to_channel.assert_called_once_with(
                "test_channel"
            )
        ...
        mock_reef.system_broadcast.assert_called_once_with(
            {"initial": "data"}, "test_channel"
        )

    @patch("praval.composition.get_reef")
    @patch("praval.composition.get_agent_info")
    def test_pipeline_trigger_returns_spore_id(
        self, mock_get_agent_info, mock_get_reef
    ):
        ...
        mock_get_agent_info.side_effect = [
            {"underlying_agent": Mock()} for _ in self.mock_agents
        ]
...
class TestConditionalAgent:
    ...
    def test_conditional_agent_requires_agent_decorator(self):
        ...
        with pytest.raises(
            ValueError,
            match="conditional_agent must be applied to @agent decorated functions",
        ):
...
class TestThrottledAgent:
    ...
    def test_throttled_agent_requires_agent_decorator(self):
        ...
        with pytest.raises(
            ValueError,
            match="throttled_agent must be applied to @agent decorated functions",
        ):
...
class TestAgentSession:
    ...
    def test_agent_session_add_agent_single(self):
        ...
        with patch("praval.composition.get_agent_info") as mock_get_info:
            ...
            # Verify subscription
            mock_agent._praval_agent.subscribe_to_channel.assert_called_once_with(
    ...
    def test_agent_session_add_agents_multiple(self):
        ...
        with patch("praval.composition.get_agent_info") as mock_get_info:
            mock_get_info.side_effect = [
                {"underlying_agent": agent._praval_agent} for agent in self.mock_agents
            ]
            ...
            for mock_agent in self.mock_agents:
                mock_agent._praval_agent.subscribe_to_channel.assert_called_with(
                    session.channel_name
                )
    ...
    def test_agent_session_broadcast(self, mock_get_reef):
        ...
        expected_data = {
            "message": "hello",
            "priority": "high",
            "_session": "broadcast_test",
        }
        mock_reef.system_broadcast.assert_called_once_with(
            expected_data, "session_broadcast_test"
        )
    ...
    def test_agent_session_get_stats_with_agents(self):
        ...
        with patch("praval.composition.get_agent_info") as mock_get_info:
            ...
            mock_get_info.side_effect = (
                mock_infos + mock_infos
            )  # Called twice (add + stats)
    ...
    def test_agent_session_fluent_interface(self):
        ...
        with patch("praval.composition.get_agent_info") as mock_get_info:
            ...
            result = session.add_agent(self.mock_agents[0]).add_agents(
                *self.mock_agents[1:]
            )
...
class TestStartAgents:
    ...
    def test_start_agents_basic_functionality(self, mock_get_agent_info, mock_get_reef):
        ...
        result = start_agents(
            *self.mock_agents, initial_data=initial_data, channel="startup_test"
        )
    ...
    def test_start_agents_default_channel(self, mock_get_agent_info, mock_get_reef):
        ...
        _ = start_agents(*self.mock_agents, initial_data={"test": "default"})
    ...
    def test_start_agents_no_initial_data(self, mock_get_agent_info, mock_get_reef):
        ...
        _ = start_agents(*self.mock_agents, channel="no_data_test")

        # Should broadcast default startup message
        mock_reef.system_broadcast.assert_called_once_with(
            {"type": "agents_started"}, "no_data_test"
        )
    ...
    @patch("praval.composition.get_reef")
    @patch("praval.composition.get_agent_info")
    def test_start_agents_handles_single_agent(
        self, mock_get_agent_info, mock_get_reef
    ):
        ...
        mock_get_agent_info.return_value = {
            "underlying_agent": self.mock_agents[0]._praval_agent
        }
        ...
        self.mock_agents[0]._praval_agent.subscribe_to_channel.assert_called_once_with(
            "startup"
        )
...
class TestIntegrationScenarios:
    ...
    @patch("praval.composition.get_reef")
    @patch("praval.composition.get_agent_info")
    def test_pipeline_with_session_integration(
        self, mock_get_agent_info, mock_get_reef
    ):
        ...
        mock_get_agent_info.side_effect = [
            {"underlying_agent": agent._praval_agent} for agent in mock_agents
        ...
        with AgentSession("integration_session") as session:
            ...
            # Trigger pipeline
            _ = pipeline({"integration": "test"})

            # Session broadcast
            _ = session.broadcast({"session": "data"})
    ...
    @patch("praval.composition.get_reef")
    @patch("praval.composition.get_agent_info")
    def test_start_agents_with_decorated_agents(
        self, mock_get_agent_info, mock_get_reef
    ):
        ...
        mock_get_agent_info.return_value = {
            "underlying_agent": base_agent._praval_agent
        }
