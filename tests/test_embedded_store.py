...
class TestEmbeddedVectorStoreInitialization:
    ...
    def test_embedded_store_default_initialization(
        ...
        with patch("praval.memory.embedded_store.CHROMADB_AVAILABLE", True):
            with patch(
                "praval.memory.embedded_store.SENTENCE_TRANSFORMERS_AVAILABLE", True
            ):
    ...
    @patch("praval.memory.embedded_store.chromadb")
    @patch("praval.memory.embedded_store.SentenceTransformer")
    def test_embedded_store_custom_initialization(
        self, mock_sentence_transformer, mock_chromadb
    ):
    ...
    def test_embedded_store_existing_collection(self, mock_chromadb):
        ...
        mock_collection.get.return_value = {
            "ids": []
        }  # Empty collection for migration check
        ...
        with patch("praval.memory.embedded_store.CHROMADB_AVAILABLE", True):
            with patch(
                ...
                _ = EmbeddedVectorStore()

        # With collection separation, should try to get both knowledge and memory
        # collections
    ...
    def test_embedded_store_chromadb_unavailable(self):
        ...
        with patch("praval.memory.embedded_store.CHROMADB_AVAILABLE", False):
            with pytest.raises(
                ImportError, match="ChromaDB is required for embedded vector store"
            ):
                EmbeddedVectorStore()
    ...
    @patch("praval.memory.embedded_store.chromadb")
    @patch("praval.memory.embedded_store.SentenceTransformer")
    def test_embedded_store_sentence_transformer_init_failure(
        self, mock_sentence_transformer, mock_chromadb
    ):
...
class TestEmbeddedVectorStoreStorage:
    ...
    def test_store_memory_basic(self, mock_sentence_transformer, mock_chromadb):
        ...
        assert metadata["meta_test"] is True
        assert metadata["meta_priority"] == "high"

    @patch("praval.memory.embedded_store.chromadb")
    @patch("praval.memory.embedded_store.SentenceTransformer")
    def test_store_memory_with_existing_embedding(
        self, mock_sentence_transformer, mock_chromadb
    ):
    ...
    def test_store_memory_complex_metadata(self, mock_chromadb):
        ...
        assert metadata["meta_bool_field"] is True
...
class TestEmbeddedVectorStoreRetrieval:
    ...
    def test_retrieve_memory_existing(self, mock_chromadb):
        ...
        self.mock_collection.get.assert_called_once_with(
            ids=["retrieve_test"], include=["metadatas", "documents", "embeddings"]
        )
...
class TestEmbeddedVectorStoreSearch:
    ...
    def test_search_with_agent_filter(self, mock_chromadb):
        ...
        query = MemoryQuery(query_text="agent search", agent_id="specific_agent")
        store.search(query)
    ...
    def test_search_with_memory_type_filter_single(self, mock_chromadb):
        ...
        query = MemoryQuery(
            query_text="type search", memory_types=[MemoryType.SEMANTIC]
        )
        store.search(query)
    ...
    def test_search_with_temporal_filter(self, mock_chromadb):
        ...
        query = MemoryQuery(
            ...
            temporal_filter={"after": after_time, "before": before_time},
        )
        store.search(query)
        ...
        if "$and" in where_clause:
            ...
            has_gte = any(
                "created_at" in c and "$gte" in c["created_at"] for c in conditions
            )
            has_lte = any(
                "created_at" in c and "$lte" in c["created_at"] for c in conditions
            )
    ...
    def test_search_similarity_threshold(self, mock_chromadb):
        ...
        response_with_low_similarity = {
            ...
            "distances": [
                [0.1, 0.6]
            ],  # Second result has low similarity (0.4 after conversion)
            "embeddings": [[[0.1] * 384, [0.2] * 384]],
        }
...
class TestEmbeddedVectorStoreEmbeddings:
    ...
    @patch("praval.memory.embedded_store.chromadb")
    @patch("praval.memory.embedded_store.SentenceTransformer")
    def test_generate_embedding_with_model(
        self, mock_sentence_transformer, mock_chromadb
    ):
    ...
    @patch("praval.memory.embedded_store.chromadb")
    @patch("praval.memory.embedded_store.SentenceTransformer")
    def test_generate_embedding_model_failure_fallback(
        self, mock_sentence_transformer, mock_chromadb
    ):
...
class TestEmbeddedVectorStoreUtilityMethods:
    ...
    def test_get_stats_success(self, mock_chromadb):
        ...
        assert (
            "conversational_memories" in stats
            or "total_memories" in stats
            or "knowledge_memories" in stats
        )
...
class TestEmbeddedVectorStoreKnowledgeIndexing:
    ...
    def setup_method(self):
        ...
        (self.knowledge_dir / "readme.md").write_text(
            "# Markdown content\nSome documentation"
        )
    ...
    def test_index_knowledge_files_success(self, mock_chromadb):
        ...
        indexed_count = store.index_knowledge_files(
            self.knowledge_dir, "knowledge_agent"
        )
