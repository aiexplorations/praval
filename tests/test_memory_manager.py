...
class TestMemoryManagerInitialization:
    ...
    def test_memory_manager_qdrant_backend_unavailable(self):
        ...
        with pytest.raises(ImportError, match="qdrant-client is required"):
            MemoryManager(agent_id="qdrant_test_agent", backend="qdrant")
    ...
    def test_memory_manager_knowledge_base_from_env(self):
        ...
        manager = MemoryManager(agent_id="env_agent", knowledge_base_path=None)
    ...
    def test_memory_manager_no_knowledge_base_env(self):
        ...
        manager = MemoryManager(agent_id="no_env_agent", knowledge_base_path=None)
    ...
    def test_memory_manager_explicit_knowledge_base_overrides_env(self):
        ...
        with patch.dict(os.environ, {"PRAVAL_KNOWLEDGE_BASE": "/env/path"}):
            manager = MemoryManager(
                agent_id="override_agent", knowledge_base_path="/explicit/path"
            )
...
class TestMemoryManagerBackendInitialization:
    ...
    def test_embedded_store_initialization_success(
        ...
        manager = MemoryManager(agent_id="embedded_test", backend="chromadb")
        ...
        mock_semantic.assert_called_once_with(long_term_memory=mock_store_instance)
    ...
    @patch("praval.memory.memory_manager.EpisodicMemory")
    @patch("praval.memory.memory_manager.SemanticMemory")
    def test_qdrant_fallback_on_embedded_failure(
        self,
        mock_semantic,
        mock_episodic,
        mock_short_term,
        ...
        manager = MemoryManager(agent_id="fallback_test", backend="auto")
    ...
    def test_embedded_store_failure_non_auto_raises(self, mock_embedded_store):
        ...
        with pytest.raises(Exception, match="ChromaDB initialization failed"):
            MemoryManager(agent_id="fail_test", backend="chromadb")
    ...
    def test_qdrant_failure_non_auto_raises(self, mock_long_term):
        ...
        with pytest.raises(Exception, match="Qdrant connection failed"):
            MemoryManager(agent_id="qdrant_fail_test", backend="qdrant")

    @patch("praval.memory.memory_manager.EmbeddedVectorStore")
    @patch("praval.memory.memory_manager.LongTermMemory")
    @patch("praval.memory.memory_manager.ShortTermMemory")
    def test_memory_only_fallback(
        self, mock_short_term, mock_long_term, mock_embedded_store
    ):
        ...
        manager = MemoryManager(agent_id="memory_only_test", backend="auto")
    ...
    def test_explicit_memory_backend(self, mock_short_term):
        ...
        manager = MemoryManager(agent_id="explicit_memory_test", backend="memory")
...
class TestMemoryManagerStoreMemory:
    ...
    def setup_method(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            self.manager = MemoryManager(agent_id="store_test_agent", backend="memory")
    ...
    def test_store_memory_basic(self):
        ...
        call_args = self.mock_short_term.store.call_args[0][
            0
        ]  # First positional argument (MemoryEntry)
    ...
    @patch("praval.memory.memory_manager.ShortTermMemory")
    @patch("praval.memory.memory_manager.SemanticMemory")
    def test_store_memory_with_semantic_subsystem(
        self, mock_semantic_class, mock_short_term, mock_embedded_store
    ):
        ...
        manager = MemoryManager(agent_id="semantic_test_agent", backend="chromadb")
...
class TestMemoryManagerSearchMemories:
    ...
    def setup_method(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            self.manager = MemoryManager(agent_id="search_test_agent", backend="memory")
            self.mock_short_term = mock_short_term_instance

    def test_search_memories_basic(self):
        """Test basic memory search."""
        query = MemoryQuery(query_text="test query", agent_id="search_test_agent")

        # Mock search result
        mock_result = MemorySearchResult(
            entries=[], scores=[], query=query, total_found=0
        )
        self.mock_short_term.search.return_value = mock_result

        result = self.manager.search_memories(query)

        # Result should be the combined result (which will just be short-term in memory-
        # only mode)
...
class TestMemoryManagerConversationTracking:
    ...
    def setup_method(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            self.manager = MemoryManager(
                agent_id="conversation_test_agent", backend="memory"
            )
            self.mock_short_term = mock_short_term_instance
    ...
    @patch("praval.memory.memory_manager.ShortTermMemory")
    @patch("praval.memory.memory_manager.EpisodicMemory")
    def test_conversation_turn_with_episodic_subsystem(
        self, mock_episodic_class, mock_short_term, mock_embedded_store
    ):
        ...
        manager = MemoryManager(agent_id="episodic_test_agent", backend="chromadb")
...
class TestMemoryManagerKnowledgeReferences:
    ...
    def setup_method(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            self.manager = MemoryManager(
                agent_id="knowledge_test_agent", backend="memory"
            )
            self.mock_short_term = mock_short_term_instance
    ...
    def test_get_knowledge_references_memory_only(self):
        ...
        references = self.manager.get_knowledge_references(
            content="Test knowledge content", importance_threshold=0.7
        )
    ...
    @patch("praval.memory.memory_manager.EmbeddedVectorStore")
    @patch("praval.memory.memory_manager.ShortTermMemory")
    def test_get_knowledge_references_with_vector_store(
        self, mock_short_term, mock_embedded_store
    ):
        ...
        manager = MemoryManager(
            agent_id="vector_knowledge_test_agent", backend="chromadb"
        )

        references = manager.get_knowledge_references(
            content="Vector knowledge content", importance_threshold=0.8
        )
...
class TestMemoryManagerUtilityMethods:
    ...
    def setup_method(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            self.manager = MemoryManager(
                agent_id="utility_test_agent", backend="memory"
            )
            self.mock_short_term = mock_short_term_instance
    ...
    def test_get_conversation_context_basic(self):
        ...
        self.mock_short_term.get_recent.assert_called_once_with(
            agent_id="utility_test_agent", limit=2
        )
    ...
    def test_get_memory_stats_memory_only(self):
        ...
        expected = {
            "agent_id": "utility_test_agent",
            "backend": "memory",
            "short_term_memory": {"total_memories": 25, "agent_memories": 10},
            "collection_name": "praval_memories",
...
class TestMemoryManagerErrorHandling:
    ...
    def test_initialization_with_invalid_backend(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            manager = MemoryManager(
                agent_id="invalid_backend_test", backend="invalid_backend"
            )
    ...
    def test_store_memory_error_handling(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            manager = MemoryManager(agent_id="error_test_agent", backend="memory")
...
class TestGetActiveBackend:
    ...
    def test_get_active_backend_memory_only(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            manager = MemoryManager(agent_id="backend_test_agent", backend="memory")
    ...
    @patch("praval.memory.memory_manager.EpisodicMemory")
    @patch("praval.memory.memory_manager.SemanticMemory")
    def test_get_active_backend_chromadb(
        self, mock_semantic, mock_episodic, mock_short_term, mock_embedded_store
    ):
    ...
    @patch("praval.memory.memory_manager.EpisodicMemory")
    @patch("praval.memory.memory_manager.SemanticMemory")
    def test_get_active_backend_qdrant(
        self,
        mock_semantic,
        mock_episodic,
        mock_short_term,
    ...
    def test_get_active_backend_returns_dict(self):
        ...
        with patch("praval.memory.memory_manager.ShortTermMemory") as mock_short_term:
            ...
            manager = MemoryManager(agent_id="dict_test_agent", backend="memory")
