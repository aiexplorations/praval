...
class MockTransport(MessageTransport):
    ...
    async def publish(
        self, topic: str, message: bytes, priority: int = 5, ttl: int = None
    ) -> None:
    ...
    def test_register_invalid_transport_class(self):
        ...
        with pytest.raises(ValueError, match="must inherit from MessageTransport"):
            TransportFactory.register_transport(
                TransportProtocol.REDIS, InvalidTransport
            )
...
class TestTransportConnectionManager:
    ...
    async def test_connection_context_manager_failure(self):
        ...
        with patch.object(
            TransportFactory, "create_transport", return_value=failing_transport
        ):
            with pytest.raises(ConnectionError):
                async with transport_connection(TransportProtocol.REDIS, config):
                    pass
...
class TestTransportPerformance:
    ...
    async def test_subscription_scalability(self):
        ...
        ), f"Slow message delivery: {delivery_duration:.3f}s"
...
class FakeAioPika:
    ...
    class _Channel:
        ...
        async def declare_queue(
            self, name, durable=True, exclusive=False, auto_delete=True
        ):
