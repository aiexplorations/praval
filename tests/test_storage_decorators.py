...
from praval.storage.exceptions import StorageConfigurationError, StorageNotFoundError
...
class TestStorageEnabledDecorator:
    ...
    @patch("praval.storage.decorators._ensure_storage_providers")
    @patch("praval.storage.decorators.get_data_manager")
    def test_sync_wrapper_preserves_existing_storage_kwarg(
        self, mock_get_dm, mock_ensure
    ):
    ...
    @pytest.mark.asyncio
    @patch(
        "praval.storage.decorators._ensure_storage_providers", new_callable=AsyncMock
    )
    @patch("praval.storage.decorators.get_data_manager")
...
class TestRequiresStorageDecorator:
    ...
    @patch("praval.storage.decorators.get_data_manager")
    def test_sync_wrapper_checks_provider_availability(
        self, mock_get_dm, mock_get_registry
    ):
    ...
    @patch("praval.storage.decorators.get_storage_registry")
    @patch("praval.storage.decorators.get_data_manager")
    def test_sync_wrapper_checks_multiple_providers(
        self, mock_get_dm, mock_get_registry
    ):
    ...
    @patch("praval.storage.decorators.get_storage_registry")
    @patch("praval.storage.decorators.get_data_manager")
    async def test_async_wrapper_checks_provider_availability(
        self, mock_get_dm, mock_get_registry
    ):
...
class TestEnsureStorageProviders:
    ...
    async def test_dict_of_providers(self, mock_ensure_single):
        ...
        config = {
            "providers": {"postgres": {"host": "localhost"}, "redis": {"port": 6379}},
    ...
    async def test_raises_when_auto_register_disabled(self):
        ...
        with pytest.raises(StorageConfigurationError) as exc_info:
            await _ensure_single_provider(
                mock_registry, "postgres", config, "test_agent"
            )
...
class TestAutoRegisterProvider:
    ...
    async def test_unknown_provider_type_raises(self):
        ...
        with pytest.raises(StorageConfigurationError) as exc_info:
            await _auto_register_provider(
                mock_registry, "unknown_provider", config, "test_agent"
            )
    ...
    async def test_registration_failure_raises(self, mock_fs_class):
        ...
        with pytest.raises(StorageConfigurationError) as exc_info:
            await _auto_register_provider(
                mock_registry, "filesystem", config, "test_agent"
            )
...
class TestDecoratorIntegration:
    ...
    @patch("praval.storage.decorators.get_storage_registry")
    @patch("praval.storage.decorators.get_data_manager")
    def test_storage_enabled_with_requires_storage(
        self, mock_get_dm, mock_get_registry
    ):
    ...
    @patch("praval.storage.decorators.get_storage_registry")
    @patch("praval.storage.decorators.get_data_manager")
    def test_requires_storage_preserves_function_metadata(
        self, mock_get_dm, mock_get_registry
    ):
