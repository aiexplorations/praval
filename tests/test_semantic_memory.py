...
and error conditions, including knowledge storage, concept relationships, and
validation.
"""
...
class TestSemanticMemoryFactStorage:
    ...
    def test_store_fact_with_all_parameters(self):
        ...
        assert call_args.metadata["related_concepts"] == [
            "programming",
            "high-level",
            "language",
        ]
    ...
    def test_store_fact_importance_calculation_long_fact(self):
        ...
        long_fact = (
            "This is a very long fact that contains detailed informatio"
            "n and goes on for quite a while, providing extensive conte"
            "xt and multiple pieces of information that could be consid"
            "ered important due to its comprehensiveness and detail."
        )

        self.memory.store_fact(
            agent_id="long_agent", fact=long_fact, domain="general", confidence=0.5
        )
    ...
    def test_store_fact_importance_calculation_capped(self):
        ...
        self.memory.store_fact(
            agent_id="capped_agent", fact=long_fact, domain="safety", confidence=1.0
        )
...
class TestSemanticMemoryConceptStorage:
    ...
    def test_store_concept_basic(self):
        ...
        result_id = self.memory.store_concept(
            ...
            definition=(
                "A subset of AI that enables computers to learn without exp"
                "licit programming"
            ),
        ...
        assert call_args.metadata["definition"] == (
            "A subset of AI that enables computers to learn without exp"
            "licit programming"
        )
    ...
    def test_store_concept_with_properties_and_relationships(self):
        ...
        result_id = self.memory.store_concept(
            ...
            definition=(
                "A subset of machine learning using neural networks with mu"
                "ltiple layers"
            ),
            domain="ai",
    ...
    def test_concept_importance_calculation_with_long_definition(self):
        ...
        long_definition = (
            "This is a very detailed and comprehensive definition that "
            "goes into great depth about the concept, explaining variou"
            "s aspects, nuances, and implications in considerable detai"
            "l."
        )
        ...
        # Should get length bonus (0.7 + 0.1 = 0.8) - use approx for floating point
        # comparison
...
class TestSemanticMemoryRuleStorage:
    ...
    def test_store_rule_basic(self):
        ...
        assert (
            call_args.metadata["rule_description"]
            == "How to respond to user help requests"
        )
...
class TestSemanticMemoryKnowledgeRetrieval:
    ...
    def test_get_knowledge_in_domain_exact_match(self):
        ...
        mock_search_result = MemorySearchResult(
            entries=domain_memories, scores=[0.9, 0.8, 0.3], query=Mock(), total_found=3
        )
        self.mock_long_term.search.return_value = mock_search_result

        result = self.memory.get_knowledge_in_domain(
            agent_id="domain_agent", domain="artificial_intelligence", limit=10
        )

        # Should return entries with exact domain match
        assert len(result) == 2
        assert all(
            entry.metadata["domain"] == "artificial_intelligence" for entry in result
        )
    ...
    def test_get_knowledge_in_domain_content_match(self):
        ...
        content_match_memory = self.create_semantic_memory(
            "content_match",
            "content_agent",
            "Artificial intelligence algorithms are used in robotics",
            "robotics",
        )

        mock_search_result = MemorySearchResult(
            entries=[content_match_memory], scores=[0.7], query=Mock(), total_found=1
        )
        self.mock_long_term.search.return_value = mock_search_result
    ...
    def test_get_knowledge_in_domain_no_matches(self):
        ...
        mock_search_result = MemorySearchResult(
            entries=[], scores=[], query=Mock(), total_found=0
        )
        self.mock_long_term.search.return_value = mock_search_result

        result = self.memory.get_knowledge_in_domain(
            agent_id="empty_agent", domain="nonexistent_domain", limit=10
        )
...
class TestSemanticMemoryConceptRelations:
    ...
    def test_find_related_concepts_basic(self):
        ...
        result = self.memory.find_related_concepts(
            agent_id="concept_agent", concept="machine learning", limit=5
        )

        # Should only return concepts and facts (not rules)
        assert len(result.entries) == 2
        assert all(
            entry.metadata["type"] in ["concept", "fact"] for entry in result.entries
        )
        assert all(
            "machine learning" in entry.content.lower() for entry in result.entries
        )
    ...
    def test_find_related_concepts_no_matches(self):
        ...
        result = self.memory.find_related_concepts(
            agent_id="no_concepts_agent", concept="nonexistent_concept", limit=5
        )
    ...
    def test_find_related_concepts_case_insensitive(self):
        ...
        mock_search_result = MemorySearchResult(
            entries=[mixed_case_memory], scores=[0.8], query=Mock(), total_found=1
        )
        self.mock_long_term.search.return_value = mock_search_result

        result = self.memory.find_related_concepts(
            agent_id="case_agent", concept="machine learning", limit=5  # lowercase
        )
...
class TestSemanticMemoryKnowledgeValidation:
    ...
    def test_validate_knowledge_supporting_evidence(self):
        ...
        mock_search_result = MemorySearchResult(
            entries=supporting_memories, scores=[0.9, 0.8], query=Mock(), total_found=2
        )
        self.mock_long_term.search.return_value = mock_search_result
    ...
    def test_validate_knowledge_contradicting_evidence(self):
        ...
        mock_search_result = MemorySearchResult(
            entries=[contradicting_memory], scores=[0.7], query=Mock(), total_found=1
        )
        self.mock_long_term.search.return_value = mock_search_result
    ...
    def test_validate_knowledge_no_evidence(self):
        ...
        assert (
            result["is_consistent"] is False
        )  # No evidence means 0 > 0 = False in implementation
...
class TestSemanticMemoryKnowledgeUpdate:
    ...
    def test_update_knowledge_existing_found(self):
        ...
        mock_search_result = MemorySearchResult(
            entries=[existing_memory], scores=[0.95], query=Mock(), total_found=1
        )
        self.mock_long_term.search.return_value = mock_search_result
        ...
        assert (
            old_update_call.metadata["update_reason"] == "Better information available"
        )
...
class TestSemanticMemoryDomainExpertise:
    ...
    def test_get_domain_expertise_level_novice(self):
        ...
        with patch.object(self.memory, "get_knowledge_in_domain", return_value=[]):
            result = self.memory.get_domain_expertise_level(
                "novice_agent", "programming"
            )
    ...
    def test_get_domain_expertise_level_intermediate(self):
        ...
        with patch.object(
            self.memory, "get_knowledge_in_domain", return_value=intermediate_knowledge
        ):
            result = self.memory.get_domain_expertise_level(
                "intermediate_agent", "programming"
            )
    ...
    def test_get_domain_expertise_level_advanced(self):
        ...
        with patch.object(
            self.memory, "get_knowledge_in_domain", return_value=advanced_knowledge
        ):
            result = self.memory.get_domain_expertise_level(
                "advanced_agent", "programming"
            )
    ...
    def test_get_domain_expertise_level_expert(self):
        ...
        # Create expert level knowledge (60 entries, 0.85 average confidence, multiple
        # domains)
        expert_knowledge = []
        ...
        with patch.object(
            self.memory, "get_knowledge_in_domain", return_value=expert_knowledge
        ):
            result = self.memory.get_domain_expertise_level(
                "expert_agent", "programming"
            )
    ...
    def test_get_domain_expertise_level_mixed_confidence(self):
        ...
        with patch.object(
            self.memory, "get_knowledge_in_domain", return_value=mixed_knowledge
        ):
    ...
    def test_get_domain_expertise_level_missing_domain_metadata(self):
        ...
        with patch.object(
            self.memory,
            "get_knowledge_in_domain",
            return_value=knowledge_with_missing_domain,
        ):
            result = self.memory.get_domain_expertise_level(
                "missing_domain_agent", "test_domain"
            )

            # Should handle missing domain metadata gracefully
            assert (
                result["expertise_level"] == "intermediate"
            )  # 15 entries, 0.7 confidence
...
class TestSemanticMemoryIntegration:
    ...
    def test_knowledge_consistency_across_types(self):
        ...
        self.mock_long_term.store.side_effect = [
            "test_fact",
            "test_concept",
            "test_rule",
        ]
        ...
        self.memory.store_concept(
            "consistency_agent", "Test Concept", "Test definition", "test"
        )
