...
class TestLongTermMemoryInitialization:
    ...
    def test_long_term_memory_default_initialization(
        self, mock_qdrant_client, mock_vector_params
    ):
    ...
    @patch("praval.memory.long_term_memory.Distance", MockDistance)
    @patch("praval.memory.long_term_memory.QdrantClient")
    def test_long_term_memory_custom_initialization(
        self, mock_qdrant_client, mock_vector_params
    ):
    ...
    def test_long_term_memory_qdrant_unavailable(self):
        ...
        with patch("praval.memory.long_term_memory.QDRANT_AVAILABLE", False):
            with pytest.raises(
                ImportError, match="qdrant-client is required for long-term memory"
            ):
                LongTermMemory()
    ...
    @patch("praval.memory.long_term_memory.Distance", MockDistance)
    @patch("praval.memory.long_term_memory.QdrantClient")
    def test_long_term_memory_distance_metric_mapping(
        self, mock_qdrant_client, mock_vector_params
    ):
        ...
        with patch("praval.memory.long_term_memory.QDRANT_AVAILABLE", True):
            with patch("praval.memory.long_term_memory.models"):
                _ = LongTermMemory(distance_metric="dot")
...
class TestLongTermMemoryStorage:
    ...
    def test_store_memory_basic(
        self, mock_qdrant_client, mock_vector_params, mock_point_struct
    ):
    ...
    def test_store_memory_qdrant_failure(
        ...
        with patch.object(
            memory, "_generate_embedding", return_value=self.mock_embedding
        ):
            entry = MemoryEntry(
                id="failure_test",
                agent_id="test_agent",
    ...
    def test_search_with_agent_filter(self, mock_qdrant_client):
        ...
        with patch.object(memory, "_generate_embedding", return_value=query_embedding):
            with patch.object(memory, "_update_access_info"):
                with patch("praval.memory.long_term_memory.models"):
                    query = MemoryQuery(
                        query_text="agent search", agent_id="specific_agent"
                    )
                    memory.search(query)
