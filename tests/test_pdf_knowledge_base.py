...
class TestPDFKnowledgeBase:
    ...
    def test_extract_pdf_text_success(self, mock_pdf_reader):
        ...
        mock_page1.extract_text.return_value = (
            "This is page 1 content with important information."
        )
        mock_page2 = Mock()
        mock_page2.extract_text.return_value = (
            "This is page 2 with more details about the topic."
        )
    ...
    def test_index_knowledge_files_with_pdf(self, mock_pdf_reader):
        ...
        _ = self.create_test_text_file("This is a text file about machine learning.")

        # Mock PDF content
        mock_page = Mock()
        mock_page.extract_text.return_value = (
            "This is PDF content about reinforcement learning and Q-lea"
            "rning algorithms."
        )
        mock_reader_instance = Mock()
        ...
        _ = self.create_mock_pdf(["RL content"], "research_paper.pdf")
        ...
        text_entry = next(
            (e for e in indexed_entries if e.metadata["file_type"] == ".txt"), None
        )
        pdf_entry = next(
            (e for e in indexed_entries if e.metadata["file_type"] == ".pdf"), None
        )
    ...
    def test_index_knowledge_files_pdf_too_short(self, mock_pdf_reader):
        ...
        _ = self.create_mock_pdf(["short"], "tiny.pdf")
    ...
    def test_index_knowledge_files_mixed_success_failure(self, mock_pdf_reader):
        ...
        _ = self.create_test_text_file("Good text content about AI research.")

        # Create a good PDF
        mock_page_good = Mock()
        mock_page_good.extract_text.return_value = (
            "This is good PDF content with sufficient length for indexing."
        )
        ...
        _ = self.create_mock_pdf(["content"], "good.pdf")
        _ = self.create_mock_pdf(["content"], "bad.pdf")
    ...
    def test_large_pdf_handling(self, mock_pdf_reader):
        ...
        for i in range(100):  # 100 pages
            mock_page = Mock()
            mock_page.extract_text.return_value = (
                f"Page {i+1} content with research data and analysis. " * 50
            )
