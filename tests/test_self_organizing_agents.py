...
class TestSystemLevelReefBroadcasting:
    ...
    def test_reef_system_broadcast(self):
        ...
        message_id = reef.system_broadcast(
            knowledge={"type": "system_start", "goal": "test"}, channel="test_channel"
        )
    ...
    def test_agent_autonomous_behavior_activation(self):
        ...
        def activation_handler(spore):
            if (
                ...
                and spore.knowledge.get("type") == "start_autonomous"
            ):
...
class TestAgentCoordination:
    ...
    def test_agents_coordinate_through_broadcasts(self):
        ...
        def explorer_handler(spore):
            if (
                spore.spore_type == SporeType.BROADCAST
                and spore.knowledge.get("type") == "start_exploration"
            ):
        ...
        def processor_handler(spore):
            if (
                spore.spore_type == SporeType.BROADCAST
                and spore.knowledge.get("type") == "discovery_made"
            ):
    ...
    def test_agents_self_organize_workload(self):
        ...
        def create_worker_handler(worker_id, capacity):
            def handler(spore):
                if (
                    spore.spore_type == SporeType.BROADCAST
                    and spore.knowledge.get("type") == "work_available"
                ):
...
class TestEmergentBehavior:
    ...
    def test_knowledge_graph_emergence(self):
        ...
        def discoverer_handler(spore):
            if (
                spore.spore_type == SporeType.BROADCAST
                and spore.knowledge.get("type") == "explore_concept"
            ):
        ...
        def relationship_handler(spore):
            if (
                spore.spore_type == SporeType.BROADCAST
                and spore.knowledge.get("type") == "concepts_discovered"
            ):
                ...
                for concept in concepts:
                    knowledge_graph["edges"].append(
                        {"from": source, "to": concept, "type": "related_to"}
                    )

                # Broadcast relationships
                relationship_finder.broadcast_knowledge(
                    {"type": "relationships_created", "count": len(concepts)},
                    channel="knowledge",
                )

        discoverer = Agent("discoverer", system_message="Discovers concepts")
        relationship_finder = Agent(
            "relationship_finder", system_message="Finds relationships"
        )
        ...
        reef.system_broadcast(
            knowledge={"type": "explore_concept", "concept": "AI"}, channel="knowledge"
        )
...
class TestPravalSimplicityPrinciples:
    ...
    def test_no_complex_inheritance_required(self):
        ...
        reef.system_broadcast(knowledge={"activate": True}, channel="simple_test")
...
class TestAsyncAgentExecution:
    ...
    def test_concurrent_agent_execution_faster(self):
        ...
        for i in range(3):
            reef.system_broadcast(
                {"type": "perf_task", "task_id": f"task_{i}"}, channel="perf_test"
            )
        ...
        # Instead, check that individual task times are reasonable (indicating parallel
        # execution)
    ...
    def test_async_agent_with_achat(self):
        ...
        for i in range(2):
            reef.system_broadcast(
                {"type": "async_task", "task_id": f"async_{i}"}, channel="async_test"
            )
    ...
    def test_message_type_filtering_with_threads(self):
        ...
        @agent(
            "filtered_processor", channel="filter_test", responds_to=["process_only"]
        )
        ...
        reef.system_broadcast(
            {"type": "process_only", "data": "should_process_1"}, channel="filter_test"
        )
        reef.system_broadcast(
            {"type": "ignore_me", "data": "should_ignore_1"}, channel="filter_test"
        )
        reef.system_broadcast(
            {"type": "process_only", "data": "should_process_2"}, channel="filter_test"
        )
        reef.system_broadcast(
            {"type": "skip_this", "data": "should_ignore_2"}, channel="filter_test"
        )
