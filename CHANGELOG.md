...
- Added provider-neutral evaluation cases, JSONL datasets, immutable subjects,
  SQLite and PostgreSQL stores, bounded runners, deterministic metrics, direct
  model judges, least-privilege evaluator-agent judges, gates, comparisons,
  and explicit baseline promotion.
- Added the `praval.eval.metrics` plugin contract, a reference metric plugin,
  and an optional RAGAS 0.4 adapter over configured Praval model and embedding
  runtimes.
- Added opt-in trace-sampled online evaluation with bounded request-path
  enqueue, durable PostgreSQL jobs, leases, retries, recovery, dead letters,
  post-hoc trace links, and metadata-only health signals.
- Added a top-level Evaluation manual, recommended target/evaluator patterns,
  credential-free exact-wheel examples, CLI operations, configuration
  reference, privacy guidance, and a v0.8.3 migration guide.
...
- `praval doctor` now reports evaluation core, SQLite, PostgreSQL, and RAGAS
  availability without printing DSNs or provider credentials.
...
Version 0.8.0 was briefly uploaded during release preparation and then
withdrawn before a matching Git tag and GitHub release existed. PyPI does not
allow deleted release filenames to be reused. Version 0.8.1 is therefore the
first supported 0.8 release. It keeps the planned 0.8 runtime behavior and adds
release diagnostics, wheel-only publication, and clearer validation guidance.
...
[the release notes](https://github.com/aiexplorations/praval/blob/main/docs/releases/RELEASE_NOTES_0.8.1.md).
...
- `praval --version`, `praval doctor`, and `praval doctor --json` for safe,
  scriptable installation and environment diagnostics.
...
- The release bundle contains one universal wheel. The build rejects source
  distributions, JSON files, and any other extra item in `dist/`. Checksums,
...
- Paid provider, real HITL, multimodal, STT, and TTS certification is optional,
  protected, and manually dispatched. It does not run on a push or pull
  request. Developers can also run the OpenAI checks with their own keys.
...
- Reef and ReefChannel shutdown are idempotent, so explicit cleanup is not
  repeated by interpreter finalization.
...
- Agent communication examples 002 and 003 now use bounded correlated flows,
  wait for Reef completion, collect worker results, and print them in order.
...
- Normal CI covers the supported Python versions, complete-package coverage,
  ...
  service demos.
- Optional live certification uses real provider APIs. Its voice path performs
...
- Release evidence records the commit, exact wheel hash, models, demo results,
  ...
  evidence.
- CI produces one exact wheel. The maintainer uploads that named wheel with
  Twine. The tag workflow then checks PyPI's SHA-256 against the CI manifest and
  attaches the same wheel to the GitHub release. It does not rebuild or upload.
